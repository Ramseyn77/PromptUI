...
export default async function LibraryPage({ searchParams }: { searchParams: Promise<{ q?: string; category?: string }> }) {
  ...
  const categoryCount = categories.filter((item) => item !== 'All').length;
  const styleCount = styles.filter((item) => item !== 'All').length;
      ...
      <div className="grid gap-8 border-b border-[var(--line)] pb-10 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
        ...
        <div className="rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-5 shadow-sm">
          ...
          <div className="mt-4 grid grid-cols-3 gap-3 text-center">
            <div><p className="text-2xl font-semibold">{components.length}</p><p className="text-xs text-[var(--muted)]">items</p></div>
            <div><p className="text-2xl font-semibold">{categoryCount}</p><p className="text-xs text-[var(--muted)]">categories</p></div>
            <div><p className="text-2xl font-semibold">{styleCount}</p><p className="text-xs text-[var(--muted)]">types</p></div>
          </div>
        </div>
      </div>
      <div className="mt-8">
